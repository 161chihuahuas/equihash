...
void Equihash::FillMemory(uint32_t length)  // works for k<=7
{
  ...
  for (unsigned i = 0; i < SEED_LENGTH; ++i) input[i] = seed[i];
...
std::vector<Input> Equihash::ResolveTreeByLevel(Fork fork, unsigned level) {
  if (level == 0) return std::vector<Input>{fork.ref1, fork.ref2};
...
void Equihash::ResolveCollisions(bool store) {
  const unsigned tableLength =
      tupleList.size();  // number of rows in the hashtable
  const unsigned maxNewCollisions =
      tupleList.size() *
      FORK_MULTIPLIER;  // max number of collisions to be found
  const unsigned newBlocks = tupleList[0][0].blocks.size() -
                             1;  // number of blocks in the future collisions
  std::vector<Fork> newForks(
      maxNewCollisions);  // list of forks created at this step
  auto tableRow =
      vector<Tuple>(LIST_LENGTH, Tuple(newBlocks));  // Row in the hash table
  vector<vector<Tuple>> collisionList(tableLength, tableRow);
  std::vector<unsigned> newFilledList(tableLength,
                                      0);  // number of entries in rows
  ...
  for (unsigned i = 0; i < tableLength; ++i) {
    for (unsigned j = 0; j < filledList[i]; ++j) {
      for (unsigned m = j + 1; m < filledList[i]; ++m) {  // Collision
        // New index
        uint32_t newIndex =
            tupleList[i][j].blocks[0] ^ tupleList[i][m].blocks[0];
        Fork newFork =
            Fork(tupleList[i][j].reference, tupleList[i][m].reference);
        ...
        if (store) {            // last step
          ...
          if (newFilledList[newIndex] < LIST_LENGTH &&
              newColls < maxNewCollisions) {
            ...
            collisionList[newIndex][newFilledList[newIndex]].reference =
                newColls;
...
Proof Equihash::FindProof() {
  ...
  while (nonce < MAX_NONCE) {
    ...
    for (unsigned i = 1; i <= k; ++i) {
      ...
      bool to_store = (i == k);
      ResolveCollisions(
          to_store);  // XOR collisions, concatenate indices and shift
    ...
    uint32_t kbytes =
        (tupleList.size() * LIST_LENGTH * k * sizeof(uint32_t)) / (1UL << 10);
    ...
    for (unsigned i = 0; i < solutions.size(); ++i) {
      ...
      for (unsigned k = 0; k < vec.size() - 1; ++k) {
        if (vec[k] == vec[k + 1]) dup = true;
      }
      if (!dup) return solutions[i];
    }
  }
